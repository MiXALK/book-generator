...
class GenerateBookTextJob implements ShouldQueue
{
    use Queueable;
    ...
    {
        $this->onQueue('generation-text');
        $this->tries = (int) config('services.observability.job_max_attempts', 3);
        $this->timeout = (int) config('services.scaling.text_job_timeout_seconds', 120);
    ...
    {
        $generationService->runTextGeneration($this->generationId);
    }
}
